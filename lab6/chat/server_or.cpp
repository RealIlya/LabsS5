...
// ====== ВНЕСЕНО ИЗМЕНЕНИЕ: Функция для вывода всех IP-адресов сервера
// Требование: "выводить на экран IP-адрес компьютера, на котором загружается серверная программа"
void print_server_info() {
    ...
    if (host != nullptr) {
        ...
        for (int i = 0; host->h_addr_list[i] != 0; ++i) {
            struct in_addr addr;
            // Копируем адрес в структуру in_addr для правильного отображения
...
SOCKET_TYPE create_listen_socket(int port) {
    SOCKET_TYPE listen_socket = socket(AF_INET, SOCK_STREAM, 0); // 0 = TCP auto
    ...
    // ====== ВНЕСЕНО ИЗМЕНЕНИЕ: Привязка к INADDR_ANY
    // Требование: "возможность обработки запросов, поступающих из любых типов сетей"
